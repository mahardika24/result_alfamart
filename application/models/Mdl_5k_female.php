...
class mdl_5k_female extends CI_Model {
 
    var $table = 'result_5k_female';
    ...
    function get_runners($per_page, $offset, $sortfield, $orderBy, $search_string, $id=0)
	{
		if(empty($id)){
			...
			$query = $this->db->get('result_5k_female');
			if ($query->num_rows() > 0) {
		...
		$query = $this->db->get_where('result_5k_female', array('id_result' => $id));
    ...
    public function record_count($search_string) {
       ...
       return $this->db->count_all_results("result_5k_female");
    }
}
